<template>
  ...
  <input v-model="formData.jurusan" type="text" class="form-control mx-3"  style="width: 250px;" id="formGroupExampleInput" placeholder="jurusan">
</div>
...
<script>
  export default{
    ...
    mounted() {
      ...
      this.$axios.get(`/api/web/jurusan/${this.$route.params.id}`)
    ...
    async updateJurusan() {
      try {
        await this.$axios.put(`/api/web/jurusan/update/${this.$route.params.id}`, this.formData);
