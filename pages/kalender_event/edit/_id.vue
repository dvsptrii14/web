<template>
  ...
  <input v-model="formData.judul" type="text" class="form-control mx-3"  style="width: 250px;" id="formGroupExampleInput" placeholder="judul">
</div>
<div class="mb-3">
  <label class="form-label p-3 my-0">tanggal</label>
  <input v-model="formData.tanggal" type="text" class="form-control mx-3"  style="width: 250px"   id="formGroupExampleInput2" placeholder="tanggal">
</div>
<div class="mb-3">
  <label class="form-label p-3 my-0">deskripsi</label>
  <input v-model="formData.deskripsi" type="text" class="form-control mx-3" style="width: 250px"  id="formGroupExampleInput2" placeholder="deskripsi">
</div>
<div class="mb-3">
  <label class="form-label p-3 my-0">tempat</label>
  <input v-model="formData.tempat" type="text" class="form-control mx-3" style="width: 250px"  id="formGroupExampleInput2" placeholder="tempat">
</div>
<div class="mb-3">
  <label class="form-label p-3 my-0">penanggung  jawab</label>
  <input v-model="formData.penanggungjawab" type="text" class="form-control mx-3" style="width: 250px"  id="formGroupExampleInput2" placeholder="penanggung jawab">
</div>
<div class="mb-3">
  <label class="form-label p-3 my-0">kontak</label>
  <input v-model="formData.kontak" type="text" class="form-control mx-3" style="width: 250px"  id="formGroupExampleInput2" placeholder="kontak">
</div>
<div class="p-3">
...
<div>
 <b-table striped hover :items = "setPenulis" :fields="field">
  <template v-slot:cell(action)="row">
    <b-button :to="{name:'kalender_event-edit-id', params:{id:row.item.id}}">Edit</b-button>
  </template>
  </b-table>
</div>
    </div>
...
<script>
  export default{
    ...
    mounted() {
      ...
      this.$axios.get(`/api/web/kalender_event/${this.$route.params.id}`)
    ...
    async updatekalender_event() {
      try {
        await this.$axios.put(`/api/web/kalender_event/update/${this.$route.params.id}`, this.formData);
