    ...
    <div class="d-flex justify-content-center align-items-center w-auto bg-success-subtle mx-auto p-">
        <div class="card bg-success-subtle" style="width: 100%;">
            ...
            <nuxt-link to="/kalender_event/create">
                <button style="border-radius: 10px" class="mx-3 my-3">Input Data Tabungan</button>
            </nuxt-link>

<div>
    <b-table striped hover :items = "settabungan" :fields="field">
    <template v-slot:cell(action)="row">
        <b-button :to="{name:'tabungan-edit-id', params:{id:row.item.id}}">Edit</b-button>
    ...
    async asyncData({$axios}){
        ...
        return{
            settabungan: getTabungan.map((item,i) => ({
                ...
                jumlah_tabungan: item.deskripsi,
                jumlah_penarikan: item.tempat,
                tanggal_penarikan: item.penanggungjawab,
                total_penarikan: item.kontak, 
